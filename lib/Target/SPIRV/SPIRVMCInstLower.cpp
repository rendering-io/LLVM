// SPIRVMCInstLower.cpp - Convert SPIRV MachineInstr to an MCInst //
//
...
///
/// \file
/// \brief This file contains code to lower SPIRV MachineInstrs to their
...
MCSymbol *
SPIRVMCInstLower::GetGlobalAddressSymbol(const MachineOperand &MO) const {
  return Printer.getSymbol(MO.getGlobal());
}

MCSymbol *SPIRVMCInstLower::GetExternalSymbolSymbol(
    const MachineOperand &MO) const {
  return Printer.GetExternalSymbolSymbol(MO.getSymbolName());
}

MCOperand SPIRVMCInstLower::LowerSymbolOperand(MCSymbol *Sym,
                                                     int64_t Offset,
                                                     bool IsFunc) const {
  MCSymbolRefExpr::VariantKind VK =
      IsFunc ? /*MCSymbolRefExpr::VK_SPIRV_FUNCTION*/ MCSymbolRefExpr::VK_None
  ...
  for (unsigned i = 0, e = MI->getNumOperands(); i != e; ++i) {
    ...
    case MachineOperand::MO_GlobalAddress:
      assert(MO.getTargetFlags() == 0 &&
             "SPIRV does not use target flags on GlobalAddresses");
    ...
    case MachineOperand::MO_ExternalSymbol:
      ...
      assert((MO.getTargetFlags() & -2) == 0 &&
             "SPIRV uses only one target flag bit on ExternalSymbols");
